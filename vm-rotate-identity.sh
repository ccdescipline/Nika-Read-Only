...
# 随机化 VM 硬件身份(反检测用),每次运行前 VM 必须已关机
# 用法: sudo ./vm-rotate-identity.sh [VM_NAME]
...
# ============================================================
# 生成函数(纯 bash,避免 tr|head SIGPIPE)
# ============================================================
_gen_alnum() {
    local len="$1"
...
echo -n "  MAC:        " ; grep -oE "<mac address='[^']+'" "$XML_FILE" | head -1 | sed "s/<mac address='//;s/'$//"
echo -n "  Disk SN:    " ; grep -oE "<serial>[^<]+" "$XML_FILE" | head -1 | sed 's/<serial>//'
echo -n "  Type 1 SN:  " ; grep -oE "type=1,[^']*serial=[A-Z0-9]+" "$XML_FILE" | sed 's/.*serial=//'
echo -n "  Type 2 SN:  " ; grep -oE "type=2,[^']*serial=[A-Z0-9]+" "$XML_FILE" | sed 's/.*serial=//'
echo -n "  Type 3 SN:  " ; grep -oE "type=3,[^']*serial=[A-Z0-9]+" "$XML_FILE" | sed 's/.*serial=//'
echo -n "  Type 17 SN: " ; grep -oE "type=17,[^']*serial=[A-Z0-9]+" "$XML_FILE" | sed 's/.*serial=//'
echo ""

# ============================================================
# 重定义 VM(保留 NVRAM 保持 Windows EFI 变量)
# ============================================================
echo "重定义 VM..."
...
echo ""
echo "=== ✓ 完成 ==="
echo "启动 VM: sudo virsh start $VM_NAME"
echo ""
echo "⚠️  提醒:MAC 和磁盘序列变了,Windows 会检测到硬件变化"
echo "   - Windows Update 可能提示需要重新激活"
echo "   - 网络位置类型可能重置为'公用'(RDP 会挡)"
echo "   - VM 内 DHCP 会重新申请 IP(iptables 转发规则要重设)"
